...
object Traversal {
   ...
   */
  def traverseSequentially[A, B, M[X] <: IterableOnce[X]](as: M[A])(f: A => Future[B])(implicit ec: ExecutionContext, cbf: CanBuildFrom[M[A], B, M[B]]): Future[M[B]] =
    foldLeftSequentially(as.toIterable)(cbf())((builder, a) => f(a).map(b => builder += b)).map(_.result())
   ...
   *         from f
   */
  def foldLeftSequentially[A, B](as: Iterable[A])(z: B)(f: (B, A) => Future[B])(implicit ec: ExecutionContext): Future[B] =
